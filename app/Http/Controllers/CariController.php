    ...
    public function index(Request $request): View
    {
        ...
        if ($tipe === 'Semua Tipe' || strtolower($tipe) === 'kos') {
            ...
            $kosanList = $kosanQuery->get()->filter(function ($kosan) use ($harga) {
                ...
                if ($harga === '< Rp 1 Juta' && $hargaMin >= 1000000) {
                    return false;
                }
                if ($harga === 'Rp 1 - 2 Juta' && ($hargaMin < 1000000 || $hargaMin > 2000000)) {
                    return false;
                }
                if ($harga === 'Rp 2 - 3 Juta' && ($hargaMin < 2000000 || $hargaMin > 3000000)) {
                    return false;
                }
                if ($harga === '> Rp 3 Juta' && $hargaMin <= 3000000) {
                    return false;
                }
        ...
        if ($tipe === 'Semua Tipe' || strtolower($tipe) === 'kontrakan') {
            ...
            $kontrakanList = $kontrakanQuery->get()->filter(function ($kontrakan) use ($harga) {
                ...
                if ($harga === '< Rp 1 Juta' && $hargaTahun >= 12000000) {
                    return false;
                }
                if ($harga === 'Rp 1 - 2 Juta' && ($hargaTahun < 12000000 || $hargaTahun > 24000000)) {
                    return false;
                }
                if ($harga === 'Rp 2 - 3 Juta' && ($hargaTahun < 24000000 || $hargaTahun > 36000000)) {
                    return false;
                }
                if ($harga === '> Rp 3 Juta' && $hargaTahun <= 36000000) {
                    return false;
                }

                $avgRating = $kontrakan->ulasan->avg('rating');
                if ((int) $kontrakan->sisa_kamar <= 0) {
                    return false;
                }
