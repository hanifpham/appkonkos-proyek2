    ...
    <div class="flex h-screen w-full overflow-hidden bg-[#f5f7fb] dark:bg-[#10192d]">
        ...
        <aside class="fixed inset-y-0 left-0 z-40 hidden {{ $sidebarWidthClass }} flex-col bg-[#163f7a] dark:bg-[#0a1224] lg:flex lg:translate-x-0">
            ...
            <nav class="flex flex-1 flex-col gap-3 px-4 py-7 overflow-y-auto">
                ...
                <a
                    href="{{ $item['url'] }}"
                    @class([ 'flex items-start rounded-full px-4 py-3.5 text-[14px] leading-5 transition-colors' , 'border border-white/10 bg-[#335b94] font-medium text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] hover:bg-[#3a659f]'=> $item['active'],
            ...
            <header class="sticky top-0 z-30 shrink-0 border-b border-slate-200 bg-white dark:border-white dark:bg-[#10192c]">
                <div class="flex min-h-[100px] items-center justify-between gap-4 px-4 py-4 sm:px-6 xl:px-8">
                    ...
                    <div class="relative flex shrink-0 items-center gap-2 sm:gap-3">
                        ...
                        @livewire('App\Livewire\SuperAdmin\NotificationBell', ['lazy' => true], 'sa-bell-' . auth()->id())
                        @else
                        @livewire('App\Livewire\Common\NotificationBell', ['lazy' => true], 'common-bell-' . auth()->id())
                        @endif
            ...
            <div class="flex-1 overflow-y-auto overflow-x-hidden">
                ...
                <div
                    ...
                    <div class="h-[3px] w-full overflow-hidden bg-slate-200/50 dark:bg-slate-800/50">
                        <div class="h-full w-full animate-[loading-bar_1.5s_ease-in-out_infinite] bg-gradient-to-r from-[#0F4C81] via-[#3B82F6] to-[#0F4C81]"></div>
                    </div>
                </div>
