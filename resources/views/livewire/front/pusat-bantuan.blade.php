<div class="bg-slate-50 dark:bg-slate-900 pb-20 transition-colors duration-300">
    ...
    <section class="relative overflow-hidden bg-gradient-to-b from-blue-900 via-[#1967d2] to-blue-600 px-4 pt-24 pb-28 sm:pt-28 sm:pb-32 sm:px-6 lg:px-8">
        {{-- Decorative Elements --}}
        ...
        <div class="relative mx-auto max-w-4xl text-center">
            <h1 class="text-2xl font-black tracking-tight text-white sm:text-4xl lg:text-6xl drop-shadow-lg">
                Pusat Bantuan APPKONKOS
            </h1>
            <p class="mt-3 text-sm font-medium text-blue-100 sm:mt-4 sm:text-lg lg:text-xl">
    ...
    <section class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 -mt-14 sm:-mt-16 relative z-10">
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4 md:gap-6">
            @php
            $categories = [
            ...
            <a href="#faq-section" class="group flex flex-col items-center rounded-2xl sm:rounded-3xl bg-white dark:bg-slate-800 p-4 sm:p-6 shadow-lg shadow-slate-200/50 dark:shadow-none ring-1 ring-slate-100 dark:ring-slate-700 transition-all duration-300 hover:-translate-y-2 hover:shadow-xl hover:shadow-blue-500/10 dark:hover:shadow-blue-900/20 hover:ring-[#1967d2]/30 dark:hover:ring-blue-500/50">
                <div class="mb-3 sm:mb-5 flex h-12 w-12 sm:h-16 sm:w-16 items-center justify-center rounded-full bg-blue-50 dark:bg-slate-700 text-[#1967d2] dark:text-blue-400 transition-colors duration-300 group-hover:bg-[#1967d2] group-hover:text-white">
                    <span class="material-symbols-outlined text-2xl sm:text-3xl transition-transform duration-300 group-hover:scale-110">{{ $cat['icon'] }}</span>
                </div>
                <h3 class="text-center text-sm sm:text-base font-bold text-slate-900 dark:text-slate-100 transition-colors group-hover:text-[#1967d2] dark:group-hover:text-blue-400">{{ $cat['title'] }}</h3>
                <p class="mt-1 text-center text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 line-clamp-2">{{ $cat['desc'] }}</p>
            </a>
            @endforeach
        </div>
    </section>
    ...
    <section id="faq-section" class="mx-auto max-w-5xl px-4 py-12 sm:py-20 sm:px-6 lg:px-8">
        <div class="text-center mb-8 sm:mb-14">
            <h2 class="text-xl sm:text-3xl lg:text-4xl font-black tracking-tight text-slate-900 dark:text-white">Pertanyaan Paling Sering Diajukan (FAQ)</h2>
            <div class="mx-auto mt-3 sm:mt-4 h-1.5 w-16 sm:w-20 rounded-full bg-[#1967d2]"></div>
        </div>

        <div x-data="{ tab: 'pencari' }" class="rounded-2xl sm:rounded-3xl bg-white dark:bg-slate-800 p-4 sm:p-6 lg:p-10 shadow-sm ring-1 ring-slate-100 dark:ring-slate-700 transition-colors duration-300">
            {{-- Tabs - stacked on mobile, horizontal on sm+ --}}
            <div class="flex justify-center border-b border-slate-200 dark:border-slate-700 overflow-x-auto">
                <nav class="-mb-px flex w-full sm:w-auto" aria-label="Tabs">
                    <button @click="tab = 'pencari'"
                        ...
                        class="flex-1 sm:flex-none border-b-2 py-3 sm:py-4 px-3 sm:px-4 text-xs sm:text-base font-bold transition-colors text-center">
                        <span class="flex items-center justify-center gap-1.5 sm:gap-2">
                            <span class="material-symbols-outlined text-base sm:text-lg">person_search</span>
                            <span class="hidden sm:inline">Pencari Kos & Kontrakan</span>
                            <span class="sm:hidden">Pencari</span>
                        </span>
                    </button>
                    <button @click="tab = 'mitra'"
                        ...
                        class="flex-1 sm:flex-none border-b-2 py-3 sm:py-4 px-3 sm:px-4 text-xs sm:text-base font-bold transition-colors text-center">
                        <span class="flex items-center justify-center gap-1.5 sm:gap-2">
                            <span class="material-symbols-outlined text-base sm:text-lg">real_estate_agent</span>
                            <span class="hidden sm:inline">Mitra Pemilik Properti</span>
            ...
            <div class="mt-6 sm:mt-10">
                {{-- KONTEN PENCARI KOS --}}
                <div x-show="tab === 'pencari'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-3 sm:space-y-4">
                    ...
                    <div x-data="{ expanded: false }" class="rounded-xl sm:rounded-2xl border border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 overflow-hidden transition-all duration-300" :class="expanded ? 'ring-1 ring-[#1967d2]/30 dark:ring-blue-500/50 bg-white dark:bg-slate-800 shadow-md' : 'hover:bg-slate-50 dark:hover:bg-slate-700/80'">
                        <button @click="expanded = !expanded" class="flex w-full items-center justify-between px-4 py-4 sm:px-6 sm:py-5 text-left focus:outline-none">
                            <span class="text-sm sm:text-base font-bold text-slate-800 dark:text-slate-200 transition-colors pr-3" :class="expanded ? 'text-[#1967d2] dark:text-blue-400' : ''">{{ $faq['q'] }}</span>
                            <span class="ml-3 sm:ml-6 flex h-7 w-7 sm:h-8 sm:w-8 shrink-0 items-center justify-center rounded-full bg-slate-100 dark:bg-slate-700 text-slate-400 dark:text-slate-400 transition-transform duration-300" :class="expanded ? 'rotate-180 bg-blue-100 dark:bg-[#1967d2]/30 text-[#1967d2] dark:text-blue-400' : ''">
                                <span class="material-symbols-outlined text-lg sm:text-xl">expand_more</span>
                            </span>
                        </button>
                        <div x-show="expanded" x-collapse x-cloak>
                            <div class="px-4 pb-4 sm:px-6 sm:pb-6 text-sm sm:text-[15px] leading-relaxed text-slate-600 dark:text-slate-400 border-t border-slate-100 dark:border-slate-700 mt-1 sm:mt-2 pt-3 sm:pt-4">
                                {!! $faq['a'] !!}
                            </div>
                        </div>
                    </div>
                ...
                <div x-show="tab === 'mitra'" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-3 sm:space-y-4">
    ...
    <section class="mx-auto max-w-5xl px-4 py-8 pb-16 sm:px-6 lg:px-8">
        <div class="relative overflow-hidden rounded-2xl sm:rounded-[3rem] bg-gradient-to-br from-blue-600 via-[#1967d2] to-[#0f4fb5] dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 px-5 py-12 sm:px-12 sm:py-20 shadow-2xl text-center transition-colors duration-300">
            ...
            <div class="relative z-10">
                <span class="mb-3 sm:mb-4 inline-block rounded-full bg-white/20 dark:bg-blue-500/20 px-3 sm:px-4 py-1 sm:py-1.5 text-[10px] sm:text-xs font-black uppercase tracking-widest text-white dark:text-blue-300">Customer Support 24/7</span>
                <h2 class="text-xl sm:text-3xl lg:text-4xl font-black tracking-tight text-white">Masih Butuh Bantuan Spesifik?</h2>
                <p class="mx-auto mt-3 sm:mt-5 max-w-2xl text-sm sm:text-lg text-blue-100 dark:text-slate-300">
                    Tim Customer Service kami selalu siap membantu menyelesaikan kendala Anda terkait sistem, pembayaran, atau akun.
                </p>
                <div class="mt-6 sm:mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
                    <a href="https://wa.me/628120000000" target="_blank" class="group flex w-full sm:w-auto items-center justify-center gap-3 rounded-full bg-white dark:bg-[#25D366] px-6 sm:px-8 py-3.5 sm:py-4 text-sm sm:text-base font-bold text-[#1967d2] dark:text-white shadow-lg transition-all duration-300 hover:-translate-y-1 hover:bg-slate-50 dark:hover:bg-[#20bd5a] hover:shadow-xl dark:hover:shadow-[#25D366]/30">
                        <svg class="h-5 w-5 sm:h-6 sm:w-6 text-[#25D366] dark:text-white transition-transform duration-300 group-hover:scale-110" fill="currentColor" viewBox="0 0 24 24">
