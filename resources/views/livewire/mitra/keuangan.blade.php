@section('mitra-title', 'Manajemen Saldo dan Pencairan')
    ...
    <section class="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)]">
        <div class="relative overflow-hidden rounded-[28px] border border-slate-200 bg-white px-7 py-8 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <div class="relative z-10">
                ...
                {{-- Skeleton Saldo Utama --}}
                <div wire:loading class="w-full animate-pulse">
                    <div class="mt-4 h-10 w-48 rounded-lg bg-slate-200 dark:bg-slate-700"></div>
                    <div class="mt-3 h-4 w-full max-w-2xl rounded bg-slate-200 dark:bg-slate-700"></div>
                    <div class="mt-2 h-4 w-3/4 max-w-2xl rounded bg-slate-200 dark:bg-slate-700"></div>

                    <div class="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
                        @for ($i = 0; $i < 3; $i++)
                            <div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 dark:border-slate-700 dark:bg-slate-800/70">
                            <div class="mb-2 h-3 w-24 rounded bg-slate-200 dark:bg-slate-700"></div>
        ...
        <div class="flex flex-col justify-between rounded-2xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-700 dark:bg-slate-800/60">
            ...
            <div class="mt-6 flex flex-col gap-3">
                <button
                    ...
                    <span wire:loading.remove wire:target="konfirmasiPencairan,mintaPencairan" class="material-symbols-outlined text-[18px]">send</span>
                    <span wire:loading wire:target="konfirmasiPencairan,mintaPencairan" class="material-symbols-outlined text-[18px] animate-spin">refresh</span>
                    <span wire:loading.remove wire:target="konfirmasiPencairan,mintaPencairan">Ajukan Pencairan</span>
                    <span wire:loading wire:target="konfirmasiPencairan,mintaPencairan">Memproses...</span>
                </button>
...
<section class="overflow-visible rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
    <div class="relative z-10 flex flex-col gap-4 border-b border-slate-200 px-6 py-5 dark:border-slate-800 lg:flex-row lg:items-center lg:justify-between">
        <div>
            ...
            <div x-data="{ open: false }" class="relative">
                ...
                <div
                    ...
                    <button
                        ...
                        @click="open = false"
                        @class([ 'flex w-full items-center justify-between rounded-xl px-4 py-3 text-left text-sm transition' , 'bg-blue-50 font-semibold text-[#0F4C81] dark:bg-blue-500/10 dark:text-blue-300'=> $filterRiwayatStatus === $value,
