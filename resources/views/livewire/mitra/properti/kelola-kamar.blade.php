@section('mitra-title', 'Kelola Unit Kos')
@section('mitra-subtitle', $kosan->nama_properti.' - atur tipe kamar, foto interior, dan nomor kamar spesifik.')
    ...
    <div class="mx-auto w-full max-w-6xl space-y-6 pb-16">
        ...
        <div class="space-y-4">
            <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
                <div class="grid gap-0 lg:grid-cols-[280px_minmax(0,1fr)]">
                    <div class="relative min-h-[250px] bg-slate-100 dark:bg-slate-950">
                        @if ($fotoProperti !== '')
                            <img src="{{ $fotoProperti }}" alt="{{ $kosan->nama_properti }}" class="h-full w-full object-cover">
                        @else
                        ...
                        <div>
                            <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                                <div class="min-w-0 flex-1">
                                    <h3 class="text-2xl font-bold text-slate-900 dark:text-white">{{ $kosan->nama_properti }}</h3>
                                    <div class="mt-2 flex items-start gap-2 text-sm text-slate-500 dark:text-slate-400">
                                        ...
                                        <span>{{ $kosan->alamat_lengkap }}</span>
                                    </div>
                                </div>

                                <div class="flex flex-col gap-2 sm:flex-row lg:flex-col lg:items-stretch">
                                    <a
                                        href="{{ route('mitra.properti.tambah-kosan', ['edit' => $kosan->id]) }}"
                            ...
                            <div class="mt-5 rounded-2xl border border-slate-100 bg-slate-50 px-4 py-4 dark:border-slate-800 dark:bg-slate-950">
                                ...
                                @if ($peraturanKos->isNotEmpty())
                                    <div class="mt-3 space-y-2.5">
                                        @foreach ($peraturanKos as $aturan)
            ...
            <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1.35fr)]">
                <div class="min-w-0 rounded-2xl border border-slate-200 bg-white px-4 py-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    ...
                    <p class="mt-2 text-2xl font-bold text-slate-900 dark:text-white">{{ $totalTipe }}</p>
                ...
                <div class="min-w-0 rounded-2xl border border-slate-200 bg-white px-4 py-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    ...
                    <p class="mt-2 text-2xl font-bold text-slate-900 dark:text-white">{{ $kamarTersedia }}</p>
                    <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Dari total {{ $totalKamar }} kamar</p>
                </div>

                <div class="min-w-0 rounded-2xl border border-slate-200 bg-white px-4 py-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    <p class="text-[10px] font-semibold uppercase tracking-[0.2em] text-amber-600 dark:text-amber-300">Kamar Dihuni</p>
                    <p class="mt-2 text-2xl font-bold text-slate-900 dark:text-white">{{ $kamarDihuni }}</p>
                ...
                <div class="min-w-0 rounded-2xl border border-slate-200 bg-white px-4 py-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    ...
                    <p class="mt-2 truncate text-[1.35rem] font-bold leading-tight text-slate-900 dark:text-white" title="{{ $kosan->harga_range ?? 'Belum diatur' }}">{{ $kosan->harga_range ?? 'Belum diatur' }}</p>
                    ...
                    <div class="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-start">
                        <div class="space-y-5">
                            <div>
                                ...
                                >
                                @error('nama_tipe') <p class="mt-2 text-sm text-rose-600">{{ $message }}</p> @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="850000"
                                >
                                @error('harga_per_bulan') <p class="mt-2 text-sm text-rose-600">{{ $message }}</p> @enderror
                            </div>
                            ...
                            <div>
                                ...
                                ></textarea>
                                @error('fasilitas_tipe') <p class="mt-2 text-sm text-rose-600">{{ $message }}</p> @enderror
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            @error('foto_interior') <p class="mt-2 text-sm text-rose-600">{{ $message }}</p> @enderror

                            @if ($foto_interior)
                                <div class="mt-4 overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700">
                                    <img src="{{ $foto_interior->temporaryUrl() }}" alt="Preview foto interior" class="aspect-[4/3] w-full object-cover">
                                </div>
                            @else
        ...
        <section class="space-y-4">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400">{{ $totalKamar }} kamar tercatat, {{ $kamarTersedia }} masih tersedia.</p>
            </div>

            @if ($kosan->tipeKamar->isEmpty())
            ...
            @else
                <div class="space-y-6">
                    @foreach ($kosan->tipeKamar as $tipeKamar)
                        ...
                        <article wire:key="tipe-kamar-{{ $tipeKamar->id }}" class="overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-md dark:border-slate-800 dark:bg-slate-900">
                            <div class="grid gap-0 xl:grid-cols-[300px_minmax(0,1fr)]">
                                <div class="border-b border-slate-200 bg-slate-100 p-4 dark:border-slate-800 dark:bg-slate-950 xl:border-b-0 xl:border-r">
                                    @if ($fotoInterior !== '')
                                        <img src="{{ $fotoInterior }}" alt="{{ $tipeKamar->nama_tipe }}" class="w-full h-32 object-cover rounded-lg">
                                    @else
                                        <div class="w-full h-32 bg-gray-100 flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-gray-300">
                                            <svg class="w-8 h-8 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                            </svg>
                                            <span class="text-[10px] text-gray-400 mt-1 uppercase font-semibold">Belum ada foto</span>
                                        </div>
                                    @endif
                                </div>
                                ...
                                <div class="p-5 sm:p-6">
                                    <div class="flex flex-col gap-4 border-b border-slate-100 pb-5 dark:border-slate-800 lg:flex-row lg:items-start lg:justify-between">
                                        <div class="min-w-0">
                                            <div class="flex flex-wrap items-center gap-2">
                                                <h4 class="text-xl font-bold text-slate-900 dark:text-white">{{ $tipeKamar->nama_tipe }}</h4>
                                                <span class="inline-flex items-center rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-[#0F4C81] dark:bg-blue-950/30 dark:text-blue-300">
                                                    Rp {{ number_format($tipeKamar->harga_per_bulan, 0, ',', '.') }} / bulan
                                                </span>
                                                <span class="inline-flex items-center rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-300">
                                                    {{ $tersediaTipe }} tersedia
                                                </span>
                                            </div>
                                            <p class="mt-3 text-sm leading-6 text-slate-500 dark:text-slate-400">{{ $tipeKamar->fasilitas_tipe }}</p>
                                        </div>

                                        <button
                                            type="button"
                                            wire:click="hapusTipeKamar({{ $tipeKamar->id }})"
                                    ...
                                    <div class="mt-5 grid gap-3 sm:grid-cols-3">
                                        <div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-800 dark:bg-slate-950">
                                            ...
                                            <p class="mt-2 text-lg font-bold text-slate-900 dark:text-white">{{ $tipeKamar->kamar->count() }}</p>
                                        </div>
                                        <div class="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 dark:border-emerald-900/40 dark:bg-emerald-950/20">
                                            ...
                                            <p class="mt-2 text-lg font-bold text-emerald-700 dark:text-emerald-300">{{ $tersediaTipe }}</p>
                                        </div>
                                        <div class="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 dark:border-amber-900/40 dark:bg-amber-950/20">
                                            ...
                                            <p class="mt-2 text-lg font-bold text-amber-700 dark:text-amber-300">{{ $dihuniTipe + $perbaikanTipe }}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950">
                                        <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                                            ...
                                            <div class="flex flex-wrap gap-2">
                                                <button
                                                    type="button"
                                                    wire:click="tambahInputKamar({{ $tipeKamar->id }})"
                                                ...
                                                <button
                                                    type="button"
                                                    wire:click="simpanKamar({{ $tipeKamar->id }})"
                                        ...
                                        <div class="mt-4 space-y-2.5">
                                            @foreach ($roomInputs[$tipeKamar->id] ?? [''] as $index => $value)
                                                <div wire:key="input-kamar-{{ $tipeKamar->id }}-{{ $index }}" class="flex flex-col gap-2 sm:flex-row sm:items-center">
                                                    <input
                                                        type="text"
                                                        wire:model.defer="roomInputs.{{ $tipeKamar->id }}.{{ $index }}"
                                                    ...
                                                    <button
                                                        type="button"
                                                        wire:click="hapusInputKamar({{ $tipeKamar->id }}, {{ $index }})"
                                        ...
                                        @error('roomInputs.'.$tipeKamar->id) <p class="mt-3 text-sm text-rose-600">{{ $message }}</p> @enderror
                                    </div>
                                    ...
                                    <div class="mt-5">
                                        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                            ...
                                            <p class="text-sm text-slate-500 dark:text-slate-400">{{ $tersediaTipe }} tersedia, {{ $dihuniTipe }} dihuni, {{ $perbaikanTipe }} perbaikan</p>
                                        </div>

                                        @if ($tipeKamar->kamar->isEmpty())
                                        ...
                                        @else
                                            <div class="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                                                @foreach ($tipeKamar->kamar as $kamar)
                                                    <div wire:key="kamar-{{ $kamar->id }}" class="rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm dark:border-slate-800 dark:bg-slate-950">
                                                        <div class="flex items-center justify-between gap-3">
                                                            <div>
                                                                <p class="text-base font-bold text-slate-900 dark:text-white">{{ $kamar->nomor_kamar }}</p>
                                                                <span @class([
                                                            ...
                                                            <button
                                                                type="button"
                                                                wire:click="hapusKamar({{ $kamar->id }})"
                                                                wire:confirm="Hapus kamar {{ $kamar->nomor_kamar }}?"
