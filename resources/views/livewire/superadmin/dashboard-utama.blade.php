...
@section('mitra-subtitle', 'Pantau pendapatan platform, transaksi Midtrans, mitra aktif, dan pencairan dana.')
    ...
    <section class="overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm dark:border-gray-700 dark:bg-slate-900">
        <div class="flex items-center justify-between border-b border-gray-100 p-6 dark:border-gray-700">
            ...
            <span class="rounded-lg border border-red-100 bg-red-50 px-3 py-1 text-[10px] font-extrabold uppercase tracking-wider text-red-600">
                {{ number_format($pencairanTertunda, 0, ',', '.') }} Perlu Tindakan
            </span>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full border-collapse text-left text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    @forelse ($listPencairan as $pencairan)
                        <tr wire:key="pencairan-{{ $pencairan->id }}">
                            <td class="px-6 py-5 font-bold text-blue-600 dark:text-blue-400">
                                {{ $this->getWithdrawalDisplayId($pencairan) }}
                            </td>
                            <td class="px-6 py-5 font-semibold text-gray-800 dark:text-gray-200">
                                {{ $pencairan->pemilikProperti?->user?->name ?? '-' }}
                            </td>
                            <td class="px-6 py-5 text-gray-600 dark:text-gray-400">
                                {{ $pencairan->pemilikProperti?->nama_bank ?? '-' }} - {{ $pencairan->pemilikProperti?->nomor_rekening ?? '-' }}
                            </td>
                            <td class="px-6 py-5 font-black text-slate-900 dark:text-white">
                                Rp {{ number_format($pencairan->nominal, 0, ',', '.') }}
                            </td>
                            <td class="px-6 py-5 text-xs text-gray-400">
                                {{ $pencairan->created_at?->diffForHumans() ?? '-' }}
                            </td>
                            <td class="px-6 py-5">
                                <span class="{{ $this->getWithdrawalStatusClasses($pencairan->status) }} rounded-lg px-2.5 py-1 text-[10px] font-black">
                                    {{ $this->getWithdrawalStatusLabel($pencairan->status) }}
                                </span>
                            </td>
                            <td class="px-6 py-5">
                                <div class="flex items-center justify-center gap-2">
                                    <button
                                        type="button"
                                        wire:click="prosesCairkan({{ $pencairan->id }})"
                                        wire:loading.attr="disabled"
                                    ...
                                    <button
                                        type="button"
                                        wire:click="tolakCairan({{ $pencairan->id }})"
                                        wire:loading.attr="disabled"
                    ...
                    @forelse ($listTransaksi as $transaksi)
                        <tr wire:key="transaksi-{{ $transaksi->id }}">
                            <td class="px-6 py-4 font-black text-gray-800 dark:text-white">
                                {{ $this->getOrderDisplayId($transaksi) }}
                            </td>
                            <td class="px-6 py-4 font-medium text-gray-700 dark:text-gray-300">
                                {{ $transaksi->pencariKos?->user?->name ?? '-' }}
                            </td>
                            <td class="px-6 py-4 text-gray-400 italic">
                                {{ $this->getNamaProperti($transaksi) }}
                            </td>
                            <td class="px-6 py-4 text-gray-600 dark:text-gray-300">
                                {{ $this->getNamaPemilik($transaksi) }}
                            </td>
                            <td class="px-6 py-4 font-black text-slate-900 dark:text-white">
                                Rp {{ number_format($this->getNominalTransaksi($transaksi), 0, ',', '.') }}
                            </td>
                            <td class="px-6 py-4">
                                <span class="rounded border border-gray-200 bg-gray-100 px-2 py-0.5 text-[9px] font-extrabold text-gray-500 dark:bg-gray-800 dark:text-gray-300 dark:border-gray-700">
                                    {{ $this->getMetodePembayaran($transaksi) }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="{{ $this->getStatusTransaksiClasses($transaksi) }} rounded-lg px-2.5 py-1 text-[10px] font-black">
                                    {{ $this->getStatusTransaksiLabel($transaksi) }}
                                </span>
                            </td>
                        </tr>
                    @empty
