    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="mt-10 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
            ...
            @foreach ($areas as $area)
                <a href="{{ route('cari', ['lokasi' => $area['nama']]) }}" class="group relative overflow-hidden rounded-2xl bg-slate-200 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md h-32 sm:h-40" id="area-{{ Str::slug($area['nama']) }}">
                    <img src="{{ $area['image'] }}" alt="{{ $area['label'] }}" class="absolute inset-0 h-full w-full object-cover transition-transform duration-500 group-hover:scale-110" loading="lazy">
                    ...
                    <div class="absolute bottom-4 left-0 right-0 px-4 text-center">
                        <span class="text-base font-bold text-white sm:text-lg">{{ $area['label'] }}</span>
                    </div>
                </a>
            @endforeach

            {{-- Card Lihat Semua --}}
            <a href="{{ route('cari') }}" class="group flex items-center justify-center rounded-2xl bg-white shadow-sm ring-1 ring-[#e5e7eb] transition-all duration-300 hover:ring-2 hover:ring-[#1967d2] hover:shadow-md h-32 sm:h-40" id="area-lihat-semua">
