...
<section class="bg-[#F8FAFC] py-16" id="rekomendasi-kos">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-end justify-between">
            ...
            <a href="{{ route('cari', ['tipe' => 'Kos']) }}" class="hidden items-center gap-1 text-sm font-semibold text-[#0EA5E9] transition hover:text-[#3B82F6] sm:inline-flex" id="kos-lihat-semua">
                Lihat Semua
                ...
                <article class="group overflow-hidden rounded-xl bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg" id="kos-card-{{ $kos->id }}">
                    <div class="relative aspect-[4/3] overflow-hidden bg-slate-100">
                        ...
                        @if($kos->sisa_kamar > 0 && $kos->sisa_kamar <= 5)
                            <span class="absolute left-3 top-3 rounded-full bg-red-500 px-3 py-1 text-xs font-bold text-white shadow-sm">Sisa {{ $kos->sisa_kamar }} Kamar</span>
                    ...
                    <div class="p-4">
                        <div class="flex items-center justify-between">
                            <span class="text-xs font-bold uppercase tracking-wide text-[#0EA5E9]">
                                {{ $kos->jenis_kos ? 'Kos ' . ucfirst($kos->jenis_kos) : 'Kos' }}
                            </span>
                            @if($kos->rating)
                            <div class="flex items-center gap-1 text-sm">
                                ...
                                <span class="font-semibold text-slate-700">{{ $kos->rating }}</span>
                            </div>
                            @endif
                        </div>
                        <h3 class="mt-2 truncate text-base font-bold text-slate-800">{{ $kos->nama }}</h3>
                        ...
                        <div class="mt-4 border-t border-slate-100 pt-3">
                            @if($kos->harga_min)
                            <p class="text-lg font-bold text-slate-800">Rp {{ number_format($kos->harga_min, 0, ',', '.') }} <span class="text-sm font-normal text-slate-500">/ bln</span></p>
                            @else
        ...
        <div class="mt-6 text-center sm:hidden">
            <a href="{{ route('cari', ['tipe' => 'Kos']) }}" class="inline-flex items-center gap-1 text-sm font-semibold text-[#0EA5E9]">
