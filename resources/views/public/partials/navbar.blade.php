...
<nav x-data="{ mobileOpen: false, cariOpen: false }" class="fixed inset-x-0 top-0 z-50 border-b border-gray-100 bg-white/90 backdrop-blur-md">
    <div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
        ...
        <div class="hidden items-center gap-8 md:flex">
            <a href="{{ route('home') }}" class="text-sm font-semibold text-slate-800 transition hover:text-[#0EA5E9]" id="nav-beranda">Beranda</a>
            ...
            <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                ...
                <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 -translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-1" class="absolute left-1/2 top-full mt-2 w-48 -translate-x-1/2 overflow-hidden rounded-xl border border-gray-100 bg-white py-1 shadow-lg">
                    <a href="{{ route('cari', ['tipe' => 'Kos']) }}" class="flex items-center gap-3 px-4 py-3 text-sm text-slate-700 transition hover:bg-sky-50 hover:text-[#0EA5E9]" id="nav-cari-kos">
                    ...
                    <a href="{{ route('cari', ['tipe' => 'Kontrakan']) }}" class="flex items-center gap-3 px-4 py-3 text-sm text-slate-700 transition hover:bg-sky-50 hover:text-[#0EA5E9]" id="nav-cari-kontrakan">
    ...
    <div x-show="mobileOpen" x-cloak x-transition class="border-t border-gray-100 bg-white px-4 pb-4 pt-2 md:hidden">
        <a href="{{ route('home') }}" class="block rounded-lg px-3 py-2 text-sm font-semibold text-slate-800 hover:bg-sky-50">Beranda</a>
        ...
        <div x-show="cariOpen" x-cloak class="ml-4 space-y-1">
            <a href="{{ route('cari', ['tipe' => 'Kos']) }}" class="block rounded-lg px-3 py-2 text-sm text-slate-600 hover:bg-sky-50 hover:text-[#0EA5E9]">Cari Kos</a>
            <a href="{{ route('cari', ['tipe' => 'Kontrakan']) }}" class="block rounded-lg px-3 py-2 text-sm text-slate-600 hover:bg-sky-50 hover:text-[#0EA5E9]">Cari Kontrakan</a>
        </div>
