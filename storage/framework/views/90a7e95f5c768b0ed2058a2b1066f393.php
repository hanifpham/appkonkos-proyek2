    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="mt-10 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4">
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $locations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $loc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a href="<?php echo e(route('cari', ['lokasi' => $loc['nama']])); ?>" class="group flex items-center gap-4 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm transition-all duration-200 hover:border-[#0EA5E9]/30 hover:shadow-md" id="kampus-<?php echo e(Str::slug($loc['nama'])); ?>">
                    <div class="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl bg-sky-50 text-[#0EA5E9] transition-colors group-hover:bg-[#0EA5E9] group-hover:text-white">
                        <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="<?php echo e($loc['ikon']); ?>"/></svg>
                    </div>
                    <div class="min-w-0">
                        <p class="truncate text-sm font-semibold text-slate-800"><?php echo e($loc['nama']); ?></p>
                        <p class="text-xs text-slate-400"><?php echo e($loc['properti']); ?></p>
                    </div>
                </a>
